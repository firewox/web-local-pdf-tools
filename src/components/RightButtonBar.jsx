...
export default function RightButtonBar() {
  ...
  const { uiTheme, setUiTheme, dark, setDark } = useTheme();
  ...
  return (
    <div className="flex items-center gap-2">
      {/* UI Design Style Switcher */}
      <ThemeSwitcher uiTheme={uiTheme} setUiTheme={setUiTheme} />

      {/* Language Selector (slim, blends with the header) */}
      <div className="relative">
        <select
          value={currentLanguage}
          onChange={(e) => changeLanguage(e.target.value)}
          className="appearance-none pl-1 pr-6 py-2 text-sm font-medium text-ink-muted hover:text-ink bg-transparent rounded-btn cursor-pointer outline-none transition-colors"
          title={t('language')}
        ...
        <div className="absolute inset-y-0 right-0 flex items-center pr-0.5 pointer-events-none text-ink-faint">
